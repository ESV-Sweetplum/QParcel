...
function kbm.keyComboCheck(keyCombo, type)
    ...
    comboList = sort(comboList, function(a, b) return a:len() > b:len() end)
    ...
    if requiresCtrl ~= ctrlHeld then return false end
    if requiresShift ~= shiftHeld then return false end
    if requiresAlt ~= altHeld then return false end
